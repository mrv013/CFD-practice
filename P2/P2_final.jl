...
مطالعه انتقال حرارت یک بعدی در فین به 4 روش FTCS، BTCS، CTCS و کرنک نیکسون
 <p>
 <p><b>عنوان درس:</b>
CFD
 <p>
...
1400/12/26
<p>
<a href="https://github.com/mrv013/CFD-practice2.git">github repository</a>
<p>
...
<div  style="direction: rtl"> <p> در این تمرین قصد داریم تا مسئله و معادلات طرح شده را به چهار روش FTCT، BTCS، CTCS و کرنک نیکسون تجزیه کرده و سپس معادلات را حل و در نهایت تحلیل خطا را انجام دهیم . </p>
<p>
...
```math
\begin{equation}
ρ.C\times \frac{\partial T(x,t)}{\partial t}=
\frac{\partial^{2} T(x,t)}{\partial x^{2}}
-\frac{hc.P}{A}\times (T(x,L)-T_{\infty})
\end{equation}
...
در دامنه از 0 تا L و زمان نامحدود با شرایط اولیه و مرزی :
</p>"""|>HTML
...
```math
\begin{equation}
k\times \frac{ \partial T(x,L)}{\partial x}\bigg|_{x=L}=hc\times (T(x,L)-T_{\infty}
\end{equation}
...
"""
<div style="direction: rtl"> <p>حال در ابتدا پارامتر های مسئله را تعریف میکنیم همان طور که  از فرمول مشخص است ما 4 نوع ثابت در نظر گرفته ایم شامل : K (ضریب رسانایی حرارت)، hc (ضریب همرفتی )و c1,c2 که ضرایب ساده کننده در فرمول هستند </p> </div>
...
# ╔═╡ 089e045c-c4ca-4d4d-b3f9-1cafa18ccf2c
md"
```math
\begin{equation}
c_{1}= ρ. C
...
# ╔═╡ 5b3c31cb-eb9e-474b-919c-a934bd3a47d7
md"
```math
\begin{equation}
c_{3}= \frac{hc.P}{A}
...
<div style="direction: rtl;padding:14px";
 #text-align:center;> <p>tau , h به ترتیب فاصله مکانی و زمانی مسئله هتند و در این بخش به تعریف آن ها اقدام میکنیم  </p> </div>
...
"""
<div div style="direction: rtl;padding:14px"> <p> شرایط مرزی در تمام روش های محاسباتی (غیز از روش CTCS) یکسان هستند به همین علت توابع آن ها قبل از هر چیز تعریف میشوند  </p> </div>
...
باید طوری تعریف شود تا به حل مسئله معنی بدهد برای همین باید در نقطه ابتدایی (x=0) دما را بر اساس شرایط مرزی قرار داد و در غیر این صورت حل مسئله بی معنی میشود </p>
<p>  تعریف ماتریس شرایط اولیه(در زمان صفر):</p>
</div>
"""|>HTML

# ╔═╡ 03c595a9-7bbb-47fa-8465-a85269ff0715
md" 
...
# ╔═╡ 9c9ef8ac-f3b2-4802-aa00-cc2b3a31cd9c
...
فین (نقطه ابتداییه شبکه) همراه ثابت فرض شده است (T(x,0)= T<sub>0</sub> ) تابع تعریف ضرایب در نقات مرزی :</p>
</div>
...
"""
<div div style="direction: rtl;padding:14px"> <p> دقت شود تابع شرایط مرزی در سمت راست باید حتما بعد از اجرا تابع ضرایب باشد چرا که بخشی از ضرایب مرزی با اصلاح ضرایب میانی تکمیل شده است </p>
<div div style="direction: rtl;padding:14px"> 
<p>  شرایط مرزی در
<b>
سمت راست
</b>
فین (نقطه انتهایی شبکه) بر اساس انتقال حرارت همرفتی تعریف شده است. </p>
</div>
...
```math
\begin{equation}
k\times \frac{ \partial T(x,L)}{\partial x}\bigg|_{x=L}=h\times (T(x,L)-T_{\infty}
\end{equation}
...
```math
\begin{equation}
T_{L+1}^{ n}= -\frac{2.hc.h}{k}T_{L}^{n}+
T_{L}^{ n}+
\frac{2.hc.h}{k}T_{\infty}
\end{equation}
...
"""<span style="direction: rtl"> <h2> انجام محاسبات با روش FTCS</h2></span>
...
"""
<div div style="direction: rtl;padding:14px"> <p> روش FTCS یک روش ضمنی است که با توجه به τ و h میتواند پاسخ پیدار یا ناپیدار داشته باشد این روش خطای مرتبه 1 دارد    </p>
</div>

"""|>HTML

# ╔═╡ c76ae0c1-ed35-4547-9fef-8f3c8a464092
"""<span style="direction: rtl"> <h3> تجزیه معادلات حاکم به روش  FTCS</h3></span>

"""|>HTML

# ╔═╡ 6743e0ca-b724-4822-94df-e8254853c842
"""
<div div style="direction: rtl;padding:14px"> <p>  معادله زیر تجزیه به روش FTCS است: </p>
</div>
...
```math
\begin{equation}
T_{i}^{ n+1}= \frac{c_{2}.τ}{c_{1}. h^{2}}T_{i-1}^{n}+
\frac{c_{1}.h^{2}-2.c_{2}.τ-2.c_{3}.τ.h^{2}}{c_{1}. h^{2}}T_{i}^{ n}+
\frac{c_{2}.τ}{c_{1}. h^{2}}T_{i+1}^{ n}+
\frac{c_{3}.τ}{c_{1}}T_{\infty}
\end{equation}
...
"""<span style="direction: rtl"> <h3> کد ها و مراحل محاسبات کامپیوتری روش FTCS  </h3></span>
...
plo1s=scatter(T1_FTCS.h_range,T1_FTCS.all_time[:,1],xlabel="ξ",ylabel="θ", label="n=$n")
...
begin
    ...
    for i in 1:nr1-1 
	  scatter!(plotc, res_FTCS[i].h_range, res_FTCS[i].all_time[:,time_step_want], label="n=$(nRange1[i])" )
    end
	plotc
end
...
plotx=scatter(res_FTCS[4].h_range, res_FTCS[4].all_time[:,time_step_want], label="n=$(nRange1[4])",xlabel="h",ylabel="FTCS in time=0.5", title="slution for diffrent gride" )
...
begin
	err=zeros(nr1-1,n)
	h3= @. 1.0/(nRange1-1)
	plo3=scatter(axis= :log,xlabel="\$ h\$",ylabel="successive Error", title="slution convergence, time=$(time_step_want*τ) s")
    for i in 1:( nr1-1 )
        i1=index2.(res_FTCS[1].h_range,res_FTCS[i].h_range[2])
	    i2=index2.(res_FTCS[1].h_range,res_FTCS[i+1].h_range[2])
		for j in 2:nRange1[1]
	    err[i,j]= abs.(res_FTCS[i+1].all_time[i2[j],time_step_want]-res_FTCS[i].all_time[i1[j],time_step_want])
		end
    end
	lable=["x=$ξ" for i in 1:1,ξ in res_FTCS[1].h_range[2:end] ]
...
τ_Range=[1,0.5, 0.25, 0.125 ]*τ

# ╔═╡ 8a790d36-de2f-485c-b476-fee78772c796
time=[1, 2, 3, 4, 5]

# ╔═╡ 3d874be8-61c8-4620-bd2b-fbdb00a9f3ab
index_tim(ξ,Δξ)=Int(ξ/Δξ)

# ╔═╡ ae7a2f50-ae54-49fa-8119-0ae062ee1a30
res_FTCS_t= [ generate_all_time_FTCS(5, hc, k, τi, c1, c3,T0,T_inf,end_time,bc) for τi in τ_Range ]
...
begin
	n_time=length(time)
	n_τ=length(τ_Range)
	err_t=zeros(n_τ-1,n_time)
	#h3= @. 1.0/(nRange1-1)
	plot_FTCS_t=scatter(axis= :log,xlabel="τ",ylabel="successive Error",legend= :bottomright,title="slution convergence, x=$((x_step_t-1)*1/(nRange1[1]-1))*L ")
    for i in 1:( n_τ-1 )
        i1=index_tim.(time,τ_Range[i])
	    i2=index_tim.(time,τ_Range[i+1])
		for j in 1:n_time
	    err_t[i,j]= abs.(res_FTCS_t[i+1].all_time[x_step_t,i2[j]]-res_FTCS_t[i].all_time[x_step_t,i1[j]])
		end
    end
	lable_t=["t=$ξ" for i in 1:1,ξ in time ]
	scatter(plot_FTCS_t, τ_Range[1:end-1], err_t[:,2:end], label=lable_t )
end

# ╔═╡ 89aea175-f4e1-4777-ba28-0c684a959295
begin
	ploFTCS_slope_t=plot(xlabel="τ",ylabel="Slope",title="slution convergence, x=$((x_step_t-1)*1/(nRange1[1]-1))*L ")
    for i in 2:n_time
		plot!(ploFTCS_slope_t,τ_Range[1:end-2], slope(err_t[:,i],τ_Range[1:end-1]),label="t=$(time[i])",lw=2)
    end
	ploFTCS_slope_t
end
...
"""<span style="direction: rtl"> <h3> تجزیه معادلات حاکم به روش  BTCS</h3></span>
...
```math
\begin{equation}
T_{i}^{ n}= -\frac{c_{2}.τ}{c_{1}. h^{2}}T_{i-1}^{n+1}+
\frac{c_{1}.h^{2}+2.c_{2}.τ+2.c_{3}.τ.h^{2}}{c_{1}. h^{2}}T_{i}^{ n+1}-
\frac{c_{2}.τ}{c_{1}. h^{2}}T_{i+1}^{ n+1}-
\frac{c_{3}.τ}{c_{1}}T_{\infty}
\end{equation}
...
"""<span style="direction: rtl"> <h3> کد ها و مراحل محاسبات کامپیوتری روش BTCS  </h3></span>
...
begin
	ploFTCS_slope=plot(xlabel="\$ h\$",ylabel="Slope",title="slution convergence, time=$(time_step_want*τ) s",legend= :bottomright)
    for i in 2:length(nRange1)
		plot!(ploFTCS_slope,h3[1:end-2], slope(err[:,i],h3[1:end-1]),label="x=$(res_BTCS[1].h_range[i])",lw=2)
    end
	ploFTCS_slope
end
...
"""
<div style="direction: rtl"> <h3>   تحلیل خطا در روش BTCS</h3></div>
...
begin
    ...
    for i in 1:nr1 
	  scatter!(plotBTCS_n, res_BTCS[i].h_range, res_BTCS[i].all_time[:,time_step_want], label="n=$(nRange1[i])" )
    end
	plotBTCS_n
end
...
begin
	err2=zeros(nr1-1,n)
	#h3= @. 1.0/(nRange1-1)
	plotBTCS_e=scatter(axis= :log,xlabel="\$ h\$",ylabel="successive Error in BTCS ",legend= :bottomright, title="slution convergence, time=$(time_step_want*τ) s")
    for i in 1:( nr1-1 )
        i1=index2.(res_BTCS[1].h_range,res_BTCS[i].h_range[2])
	    i2=index2.(res_BTCS[1].h_range,res_BTCS[i+1].h_range[2])
		for j in 2:nRange1[1]
	    err2[i,j]= abs.(res_BTCS[i+1].all_time[i2[j],time_step_want]-res_BTCS[i].all_time[i1[j],time_step_want])
		end
    end
	lable2=["x=$ξ" for i in 1:1,ξ in res_BTCS[1].h_range[2:end] ]
	scatter(plotBTCS_e, h3[1:end-1], err2[:,2:end], label=lable2 )
end

# ╔═╡ f3e47cfe-9317-497e-b2ce-3ba4a8512e58
begin
	ploBTCS_slope=plot(xlabel="\$ h\$",ylabel="Slope", title="slution convergence, time=$(time_step_want*τ) s")
    for i in 2:length(nRange1)
		plot!(ploBTCS_slope,h3[1:end-2], slope(err2[:,i],h3[1:end-1]),label="x=$(res_BTCS[1].h_range[i])",lw=2)
    end
	ploBTCS_slope
end

# ╔═╡ a58c55f3-4f68-4c96-84fa-4b5e222fb81e
res_BTCS_t= [ generate_all_time_BTCS(5, hc, k, τi, c1, c3,T0,T_inf,end_time,bc) for τi in τ_Range ]

# ╔═╡ efd65b03-1aab-4274-a54b-3147d9bc2bfb
begin
	#n_time=length(time)
	#n_τ=length(τ_Range)
	err_t2=zeros(n_τ-1,n_time)
	#h3= @. 1.0/(nRange1-1)
		plot_BTCS_t=scatter(axis= :log,xlabel="τ",ylabel="successive Error", title="slution convergence, x=$((x_step_t-1)*1/(nRange1[1]-1))*L ",legend= :topleft)
    for i in 1:( n_τ-1 )
        i1=index_tim.(time,τ_Range[i])
	    i2=index_tim.(time,τ_Range[i+1])
		for j in 1:n_time
	    err_t2[i,j]= abs.(res_BTCS_t[i+1].all_time[x_step_t,i2[j]]-res_BTCS_t[i].all_time[x_step_t,i1[j]])
		end
    end
	lable_t2=["t=$ξ" for i in 1:1,ξ in time ]
	scatter(plot_BTCS_t, τ_Range[1:end-1], err_t2[:,2:end], label=lable_t2 )
end

# ╔═╡ fc4d5a24-3d19-43b1-a313-c7333799cd33
begin
	ploBTCS_slope_t=plot(xlabel="τ",ylabel="Slope",legend= :topleft,title="slution convergence, x=$((x_step_t-1)*1/(nRange1[1]-1))*L ")
    for i in 2:n_time
		plot!(ploBTCS_slope_t,τ_Range[1:end-2], slope(err_t2[:,i],τ_Range[1:end-1]),label="t=$(time[i])",lw=2)
    end
	ploBTCS_slope_t
end

# ╔═╡ a030b0f5-6d4a-415c-99f5-dc65c56ef90b
"""<div style="direction: rtl"> <h2>   انجام محاسبات با روش: CTCS</h2> </div>
<div div style="direction: rtl;padding:14px">  
...
"""<span style="direction: rtl"> <h3> تجزیه معادلات حاکم به روش  CTCS</h3></span>
...
```math
\begin{equation}
T_{i}^{ n+2}= \frac{2.c_{2}.τ}{c_{1}. h^{2}}T_{i-1}^{n+1}-
\frac{2.(2.c_{2}.τ+c_{3}.τ.h^{2})}{c_{1}. h^{2}}T_{i}^{ n+1}+
\frac{2.c_{2}.τ}{c_{1}. h^{2}}T_{i+1}^{ n+1}+
T_{i}^{ n}+
\frac{c_{3}.τ}{c_{1}}T_{\infty}
\end{equation}
...
"""<span style="direction: rtl"> <h3> کد ها و مراحل محاسبات کامپیوتری روش CTCS  </h3></span>
...
# ╔═╡ 0be7cfc1-6c73-45f0-96d4-a83dfd8ded7e


# ╔═╡ 7ecebffa-9a96-43a0-928d-d3b943879452
"""
<div style="direction: rtl"> <h3>   تحلیل خطا در روش CTCS</h3> </div>
...
"""<span style="direction: rtl"> <h3> تجزیه معادلات حاکم به روش کرنک نیکسون</h3></span>
...
```math
\begin{equation}
 c_{2}.τ\times T_{i-1}^{n+1}-
(2.c_{1}.h^{2}+2.c_{2}.τ+c_{3}/2)\times T_{i}^{ n+1}+
c_{2}.τ\times T_{i+1}^{ n+1}
...
```math
\begin{equation}
= -c_{2}.τ\times T_{i-1}^{n}+
(-2.c_{1}.h^{2}+2.c_{2}.τ+c_{3}/2)\times T_{i}^{ n}-
c_{2}.τ\times T_{i+1}^{ n}-
2.c_{3}.τ.h^{2}\times T_{\infty}
\end{equation}
```
"

# ╔═╡ 7bd41cf4-ed3c-40fd-8ac1-86f4bb448f22
"""<span style="direction: rtl"> <h3> کد ها و مراحل محاسبات کامپیوتری روش کرنک نیکسون  </h3></span>

"""|>HTML

# ╔═╡ 58c2cca9-88d8-4c9c-8205-b19d47ce29c3
function internal_coeffs_KN!(I, J, V_n1, V_n2, C, n, h, τ, c1, k, c3, T_inf )
	for i= 2:(n-1)
		K=3*(i-2).+(1:3)# [3*n+1 ][3*o+1, 3*o+2, 3*o+3]
		I[K] .= i#[i, i, i]
		J[K] = [i-1, i, i+1]
		#V[K] = [2*k*τ/c1/h^2, -2*(2*k*τ/c1/h^2+c3*τ/c1), 2*k*τ/c1/h^2]
		V_n2[K] = [k*τ, (-2*c1*h^2-2*k*τ-c3/2), k*τ]# n+1
		V_n1[K] = [-k*τ, (-2*c1*h^2+2*k*τ+c3/2), -k*τ]# n
...
function generate_les_KN(n, hc, k, τ, c1, c3, T_inf,bc)
    h=dh(n)
	lbc,rbc=bc
	In1, Jn1, Vn1, Cn1= initilize_variables(n)
	
    Vn2=copy(Vn1)
	internal_coeffs_KN!(In1, Jn1, Vn1, Vn2, Cn1, n, h, τ, c1, k, c3, T_inf )
	#lbc(In1, Jn1, Vn1, Cn1, n)
	In2 =copy(In1)
	Jn2=copy(Jn1)
	Cn2=copy(Cn1)
	lbc(In1, Jn1, Vn1, Cn1, n)
	rbc(In1, Jn1, Vn1, Cn1, n, h, hc, k, T_inf)
	lbc(In2, Jn2, Vn2, Cn2, n)
	rbc(In2, Jn2, Vn2, Cn2, n, h, hc, k, T_inf)
	Cn1[n]= Cn1[n]-(Cn2[n]-Cn2[n-1])
	return sparse(In1, Jn1, Vn1), sparse(In2, Jn2, Vn2), Cn1#,Cn2,Vn1
end

# ╔═╡ 8ef1cf23-f3a7-4ea6-95b5-61cab87f039b
"""<span style="direction: rtl"> <h4> چک کردن توابع با انجام تک مرحله   </h4></span>

"""|>HTML

# ╔═╡ 04667d1c-d58b-4ef8-b2af-493d530ed918
"""
<div div style="direction: rtl"> <p>  این زیر فصل بعد از هر بار تعریف توابع generate_les مورد استفاده برای چک کردن درستی کد ها بوده است  اما در نهایت بعد از منطقی بودن پاسخ ها حذف شده است. اما در روش گرنک نیکسون به دلیل پاسخ های غیر منطقی باقی گذاشته شده است .
...
# ╔═╡ a1c66807-1fe4-41e0-8997-436d6927a711
ho=5

# ╔═╡ 4938f673-c9c5-4aad-b358-10b0395e0036
A_kn1,A_kn2, C_kn=generate_les_KN(ho, hc, k, τ, c1, c3, T_inf,bc)

# ╔═╡ 4e5a6248-284d-42f5-9d0b-7e826a15487d
C_kn

# ╔═╡ 89ade5cc-5bfe-42e1-aec5-3f57353431e6
time1=[T0;ones(ho-1,1)*T_inf]

# ╔═╡ 84de0ab9-d481-47b9-9229-bbeebae3b45a
"""
<div div style="direction: rtl"> <p>  همان طور که مشاهده میشود پاسخ 1 مرحله زمانی برای این روش منطقی نیست (با توجه به شرایط اولیه) پس دردر همین مرحله متوقف میشویم  
